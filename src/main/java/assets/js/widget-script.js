 ...
 */
function appendToSelect(cleanOption, list, gadgetId, targetId) {
    if (cleanOption == true) {
        cleanSelect(gadgetId, targetId);
    }
    for (var z = 0; z < list.length; z++) {
        $('<option value="' + list[z] + '" selected>' + list[z] + '</option>')
            .appendTo($(gadgetId).find(targetId));
    }
}
...
function cleanSelect(targetId) {
    $(gadgetId).find(targetId).find('option').remove().end();
}
